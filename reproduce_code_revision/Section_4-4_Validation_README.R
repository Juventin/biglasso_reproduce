## -----------------------------------------------------------------------------
## Replication code accompanying the JSS paper "The biglasso Package: A Memory-
## and Computation-Efficient Solver for Lasso Model Fitting with Big Data in R"
## Authors: Yaohui Zeng and Patrick Breheny
##
## benchmarking platform:
##    MacBook Pro with Intel Core i7 @ 2.3 GHz and 16 GB RAM.
##
## -----------------------------------------------------------------------------

## -----------------------------------------------------------------------------
## Section 4.4: Validation

## Replicating Table 5

# --------------------------------------------------------------
# Linear regression: MNIST
#
# See Line 270 - 459 in R file "Section_4-2_Linear_real_data.R" 
#
# --------------------------------------------------------------
# Linear regression: GWAS
#
# See Line 82 - 269 in R file "Section_4-2_Linear_real_data.R" 
#
# --------------------------------------------------------------

# --------------------------------------------------------------
# Logistic regression: P53
#
# See Line 456 - 522 in R file "Section_4-3_Logistic_real_data.R" 
#
# --------------------------------------------------------------
# Logistic regression: NEWS20
#
# See Line 525 - 597 in R file "Section_4-3_Logistic_real_data.R" 
#
# --------------------------------------------------------------
